...
class halo(constants):
    ...
    def init_sigma_spl(self):
        ...
        Rarr = np.logspace(-2,np.log10(5*self.r_200),70)
        Sigmaarr = Rarr*0.0
    ...
    def init_sigma_cir_spl(self,r0):
        ...
        rdbin = np.logspace(-2,np.log(5*self.r_200),50)
        des_cir = 0.0*rdbin
...
if __name__ == "__main__":
    ...
    rdbin = np.logspace(-2,np.log10(2*h_p.r_200),50)
   ...
   # print h_p.delta_sigma_dau(rdbin[0],rd_dist)
    ...
    for i in range(0,len(rdbin)):
        ...
        delta_daug[i] = h_d.delta_sigma(rdbin[i])
    ...
    plt.plot(rdbin,delta_daug/1e12 ,'or',label = 'daughter')
